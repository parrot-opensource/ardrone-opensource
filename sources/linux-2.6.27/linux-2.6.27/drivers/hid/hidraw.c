...
static ssize_t hidraw_send_report(struct file *file, const char __user *buffer, size_t count)
{
	...
	if (!hidraw_table[minor]) {
		printk(KERN_WARNING "hidraw: NULL hidraw_table entry\n");
		ret = -ENODEV;
		goto out;
	}
...
static ssize_t hidraw_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	ret = hidraw_send_report(file, buffer, count);
...
/* This function performs a Get_Report transfer over the control endpoint
   ...
   mutex held.  */
static ssize_t hidraw_get_report(struct file *file, char __user *buffer, size_t count)
{
	unsigned int minor = iminor(file->f_path.dentry->d_inode);
	struct hid_device *dev;
	__u8 *buf;
	...
	ret = dev->hid_get_raw_report(dev, buf, count);
	...
	switch (cmd) {
		...
		default:
				...
				if (_IOC_NR(cmd) == _IOC_NR(HIDIOCSFEATURE(0))) {
					...
					ret = hidraw_send_report(file, user_arg, len);
					break;
				}
				if (_IOC_NR(cmd) == _IOC_NR(HIDIOCGFEATURE(0))) {
					int len = _IOC_SIZE(cmd);
					ret = hidraw_get_report(file, user_arg, len);
					break;
				}
