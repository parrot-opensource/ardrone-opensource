 ...
 *
 * TODO: check what is available on P6 and what is available on P6i
 */
    ...
    switch (cmd)
    {
        ...
        *---------------------------------------------------------------------------*/
        ...
        case AAI_IOW_PCM0MAST:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0)
                aai_setbit(aai, AAI_PCM0_CFG, AAI_PCM_CONF_MAST, (!!val));
            break;
        ...
        case AAI_IOW_PCM0_OKI:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0)
                aai_setbit(aai, AAI_PCM0_CFG, AAI_PCM_CONF_OKI, (!!val));
            break;
        ...
        case AAI_IOW_PCM0_1MHZ:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0)
                aai_setbit(aai, AAI_PCM0_CFG, AAI_PCM_CONF_1MHZ, (!!val));
            break;

        /** <li><b>AAI_IOW_PCM_CYCLESHIGH:</b><br>
         * Set PCM number of bit clock cycle per frame at 16 kHz<br>
         * Param  : integer value between 0x10 and 0x80<br>
         * Return : none
         * <hr>
         */
        case AAI_IOW_PCM_CYCLESHIGH:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0 && 0x10 <= val && val <= 0x80)
                aai_set_nbit(aai, AAI_PCM0_CFG, 0xff, AAI_PCM_CONF_CYCLES_HIGH_SHIFT, val);
            break;

        /** <li><b>AAI_IOW_PCM_CYCLESLOW:</b><br>
         * Set PCM number of bit clock cycle per frame at 8 kHz<br>
         * Param  : integer value between 0x10 and 0x100<br>
         * Return : none
         * <hr>
         */
        case AAI_IOW_PCM_CYCLESLOW:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0 && 0x10 <= val && val < 0x100)
                aai_set_nbit(aai, AAI_PCM0_CFG, 0xff, AAI_PCM_CONF_CYCLES_LOW_SHIFT, val);
            break;
        ...
        /** <li><b>AAI_IOW_CODEC_REC_SELECTION:</b><br>
         * Set P6i internal codec input PGA<br>
         * Param  : input to set
         * Return : none
         * <hr>
         */
        case AAI_IOW_CODEC_REC_SELECTION:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0)
                err = aai_codec_set_recsel(aai, val);
            break;

        /** <li><b>AAI_IOW_CODEC_I2S_FREQ:</b><br>
         * Set P6i internal codec i2s sampling frequency<br>
         * Param  : frequency to set
         * Return : none
         * <hr>
         */
        case AAI_IOW_CODEC_I2S_FREQ:
            err = get_data((void __user *)arg, &val, sizeof(unsigned int));
            if (err == 0)
                err = aai_codec_set_i2sfreq(aai, val);
            break;
