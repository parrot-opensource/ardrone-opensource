                                  ...
                                  struct snd_ctl_elem_info *uinfo)
{
    ...
    switch (kcontrol->private_value - AAI_NB_CHANNELS)
    {
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
    ...
    aai_dbg(aai->dev, " >>> %s: reg ctl3 = 0x%x\n", __FUNCTION__, reg);
...
static struct snd_kcontrol_new aai_alsa_controls[] =
{
    ...
    CTL_VOLUME_CODEC("Codec PGA Input Volume",       AAI_NB_CHANNELS + CODEC_PGA_INPUT),
    CTL_VOLUME_CODEC("Codec Record Volume",          AAI_NB_CHANNELS + CODEC_RECORD),
    CTL_SWITCH_CODEC("Codec PGA Input Switch",       AAI_NB_CHANNELS + CODEC_PGA_INPUT),
    CTL_SWITCH_CODEC("Codec Record Switch",          AAI_NB_CHANNELS + CODEC_RECORD),
    CTL_SWITCH_CODEC("Codec Output Playback Switch", AAI_NB_CHANNELS + CODEC_PLAYBACK)
};
