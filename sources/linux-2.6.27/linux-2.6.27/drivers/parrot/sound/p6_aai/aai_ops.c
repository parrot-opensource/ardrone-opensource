...
static void devdma_hw_free(struct snd_pcm_substream *substream)
{
    ...
    {
        aai_info(chan2dev(chan),"%s(%d) - free runtime->dma_area == NULL\n", chan->name, chan->ipcm);
        return;
    }
    ...
    {
        aai_info(chan2dev(chan),"%s(%d) - nothing freed\n", chan->name, chan->ipcm);
    }
...
static int devdma_hw_alloc(struct snd_pcm_substream *substream, size_t size)
{
 ...
 free:
    kfree(buf);
 nomem:
    aai_info(chan2dev(chan),"%s(%d) - allocate failed\n",
                             chan->name, chan->ipcm);
...
{
    int nbchans, format, bufsize, err= 0;
    uint32_t access;
    aai_device_t * chan = substream2chan(substream);
    struct snd_pcm_runtime *runtime = substream->runtime;
