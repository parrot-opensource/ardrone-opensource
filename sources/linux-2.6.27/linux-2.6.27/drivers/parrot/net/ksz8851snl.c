...
static void ksz8851snl_rx_handler(struct net_device *dev)
{
	...
	frame_count >>= 8;
	priv->stats.rx_packets += frame_count;
 ...
 * onto the network
 */
static int ksz8851snl_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	struct ksz8851snl_net *priv = netdev_priv(dev);
...
static void ksz8851snl_tx_timeout(struct net_device *dev)
{
	struct ksz8851snl_net *priv = netdev_priv(dev);

	if (netif_msg_timer(priv))
		printk(DRV_NAME " tx timeout\n");
...
static int ksz8851snl_net_close(struct net_device *dev)
{
	struct ksz8851snl_net *priv = netdev_priv(dev);
 ...
 */
static void ksz8851snl_set_multicast_list(struct net_device *dev)
{
	struct ksz8851snl_net *priv = netdev_priv(dev);
...
static void ksz8851snl_setrx_work_handler(struct work_struct *work)
{
	...
	if (dev->flags & IFF_PROMISC) {
		...
		for (i = 0, mclist = dev->mc_list; mclist && i < dev->mc_count;
			...
			u32 bit_nr = ether_crc(ETH_ALEN, mclist->dmi_addr) >> 26;
...
static void ksz8851snl_restart_work_handler(struct work_struct *work)
{
	struct ksz8851snl_net *priv =
			container_of(work, struct ksz8851snl_net, restart_work);
	struct net_device *dev = priv->netdev;
	int ret;

	rtnl_lock();
	if (netif_running(dev)) {
		ksz8851snl_net_close(dev);
		ret = ksz8851snl_net_open(dev);
		if (unlikely(ret)) {
			printk(" could not restart %d\n", ret);
			dev_close(dev);
		}
	}
	rtnl_unlock();
...
static struct net_device_stats * ksz8851snl_get_stats(struct net_device *dev)
{
	struct ksz8851snl_net *priv = netdev_priv(dev);
...
static int __devinit ksz8851snl_probe(struct spi_device *spi)
{
	...
	INIT_WORK(&priv->restart_work, ksz8851snl_restart_work_handler);
	...
	dev->irq = spi->irq;
	dev->open = ksz8851snl_net_open;
	dev->stop = ksz8851snl_net_close;
	dev->hard_start_xmit = ksz8851snl_send_packet;
	dev->set_multicast_list = ksz8851snl_set_multicast_list;
	dev->set_mac_address = ksz8851snl_set_mac_address;
	dev->get_stats = ksz8851snl_get_stats;
	dev->tx_timeout = ksz8851snl_tx_timeout;
