...
static int parrot5_i2cm_reset(struct parrot5_i2cm_data *drv_data)
{
	...
	status = readl(drv_data->base + I2CM_STATUS);
	if (drv_data->stop_sent && (status & I2CM_STATUS_BUSY)) {
...
static irqreturn_t parrot5_i2cm_irq(int irq, void *dev_id)
{
	...
	if (drv_data->status) {
		/* abort transfert : we didn't submit stop if we are here */
		drv_data->state = STOP;
		eprintk("i2c nak\n");
	}
				    ...
				    int num)
{
	...
	/* don't send stop here, because if will make the wait_event_timeout
	   exit
	 */
	...
	if (time_left == 0) {
		...
		eprintk("cmd failed : %d\n", drv_data->status);
		for (ret = 0; ret < num; ret++) {
			int i;
			eprintk("master_xfer[%d] %c, addr=0x%02x, "
				"len=%d%s\n", ret, (msgs[ret].flags & I2C_M_RD)
				? 'R' : 'W', msgs[ret].addr, msgs[ret].len,
				(msgs[ret].flags & I2C_M_RECV_LEN) ? "+" : "");
			for (i = 0; i < msgs[ret].len; i++)
				eprintk("buf[%d]=0x%02x\n", i, msgs[ret].buf[i]);
		}
		dump_stack();
		ret = -EREMOTEIO;
	}
