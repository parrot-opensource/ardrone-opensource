...
#define PARROT5_I2CM_TIMEOUT 1000 // in ms
...
static int parrot5_i2cm_wait_transfert(struct parrot5_i2cm_data *drv_data)
{
	u32 status;
	int ret = -1;
	...
	if (ret == 0 && (status & I2CM_STATUS_IF))
		writel(I2CM_COMMAND_ITACK, drv_data->base + I2CM_COMMAND);
...
{
	/* in case of AL, clear BUSY bit */
	if (status & I2CM_STATUS_AL)
		status &= ~I2CM_STATUS_BUSY;

	/* reset if status is not clean */
	return (status & ~(I2CM_STATUS_RXACK|I2CM_STATUS_AL));
}

/* reset controller state */
static int parrot5_i2cm_reset(struct parrot5_i2cm_data *drv_data)
{
	u32 status;
	int ret;
	/* disable irq */
	...
	/* wait a stable state */
	ret = parrot5_i2cm_wait_transfert(drv_data);
	if (ret) {
...
static irqreturn_t parrot5_i2cm_irq(int irq, void *dev_id)
{
	...
	if ((status & I2CM_STATUS_RXACK) && drv_data->state != DATA_R &&
	...
	switch (drv_data->state) {
		...
		break;
		case RESTART:
			parrot5_i2cm_start_message(drv_data, msg);
		break;
		case STOP:
			parrot5_i2cm_send_stop(drv_data);
		break;
				    ...
				    int num)
{
	...
	int ret = num;
	int timeout = msecs_to_jiffies(drv_data->adapter.timeout);
	int time_left;
	u32 status;

	/* TODO retries management if needed */
	status = readl(drv_data->base + I2CM_STATUS);
	if (parrot5_i2cm_need_reset(status)) {
		printk("bad status before transfert %x, resetting\n", status);
		parrot5_i2cm_reset(drv_data);
		status = readl(drv_data->base + I2CM_STATUS);
	...
	parrot5_i2cm_start_message(drv_data, msgs);
	time_left = wait_event_timeout(drv_data->wait,
							     drv_data->wait_up,
							     timeout);

	dprintk("xfer timeout %d\n", time_left);
	dprintk("xfer end  hw status %x, status %d\n",
			readl(drv_data->base + I2CM_STATUS),
			drv_data->status);
