...
static int goldfish_fb_ioctl(struct fb_info *info,
			     unsigned int cmd, unsigned long arg)
{
	unsigned long irq_flags;
	void __user *argp = (void __user *)arg;
	struct goldfish_fb *fb = container_of(info, struct goldfish_fb, fb);
	static struct p6fb_plane_info pi = {0, 0, 0, 0, 0};
	u8 alpha;
	int ret = 0;
	...
	case P6FB_QUERY_PLANE:
		if (copy_to_user(argp, &pi, sizeof(pi)))
			ret = -EFAULT;
		break;

	case P6FB_SETUP_PLANE:
		if (copy_from_user(&pi, argp, sizeof(pi)))
			ret = -EFAULT;
		alpha = (pi.osdmode == P6FB_OSDMODE_NONE)? 0 : pi.osdplane;

		/* update alpha */
		spin_lock_irqsave(&fb->lock, irq_flags);
		writel(alpha, fb->reg_base + FB_SET_ALPHA);
		spin_unlock_irqrestore(&fb->lock, irq_flags);
		/*printk("goldfish_fb_ioctl: set alpha = %d\n", pi.osdplane);*/
		break;
