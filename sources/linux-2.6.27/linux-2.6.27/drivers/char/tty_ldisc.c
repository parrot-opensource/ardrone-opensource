...
restart:
	...
	if (tty->ldisc.refcount || (o_tty && o_tty->ldisc.refcount)) {
		...
		if (o_tty && o_tty->ldisc.refcount) {
			/* XXX avoid some deadlock on ppp
			   ldisc.ops->name is ppp but n_tty
			 */
			if (strcmp(o_tty->ldisc.ops->name, "n_tty") == 0 &&
					o_tty->driver->type == TTY_DRIVER_TYPE_PTY &&
					o_tty->ldisc.ops == tty->ldisc.ops)
				goto ppp_skip;
			...
			goto restart;
ppp_skip:
			printk(KERN_ERR "ppp deadlock avoided\n");
		}
	}
	/*
