 ...
 */
int global_irq;
static int __devinit ci13xxx_probe(struct platform_device *pdev)
{
	void __iomem *regs = NULL;
	int retval = 0;
	int irq;
...

{
	...
	/*if (!request_mem_region(res->start, res->end - res->start + 1, UDC_DRIVER_NAME)) {
	...
	platform_set_drvdata(pdev, (__force void *)regs);
	...
	retval = request_irq(irq, ci13xxx_pci_irq, IRQF_SHARED,
			     UDC_DRIVER_NAME, pdev);
	if (retval)
		goto gadget_remove;
 ...
 * all PCI resources allocated for this USB device controller
 */
static int __devexit ci13xxx_remove(struct platform_device *pdev)
{
	free_irq(global_irq, pdev);
	udc_remove();
	iounmap((__force void __iomem *)platform_get_drvdata(pdev));
	return 0;
}

static struct platform_driver ci13xxx_plat_driver = {
	.probe = ci13xxx_probe,
	.remove = ci13xxx_remove,
	.driver = {
		   .name = "p6i-ehci",
	},
};

static struct platform_driver ci13xxx1_plat_driver = {
	.probe = ci13xxx_probe,
	.remove = ci13xxx_remove,
	.driver = {
		   .name = "p6i-ehci-dev",
...
static int __init ci13xxx_pci_init(void)
{
	platform_driver_register(&ci13xxx1_plat_driver);
	return platform_driver_register(&ci13xxx_plat_driver);
}
module_init(ci13xxx_pci_init);
...
static void __exit ci13xxx_pci_exit(void)
{
	platform_driver_unregister(&ci13xxx1_plat_driver);
	platform_driver_unregister(&ci13xxx_plat_driver);
}
module_exit(ci13xxx_pci_exit);
